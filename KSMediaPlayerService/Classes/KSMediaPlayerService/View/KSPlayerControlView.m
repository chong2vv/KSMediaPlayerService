...
#import "KSPlayerControlView.h"
#import <AVKit/AVKit.h>
#import <AVFoundation/AVFoundation.h>
#import <ArtCommon/UIView+ZFFrame.h>
#import <ArtCommon/ZFSliderView.h>
#import <ArtCommon/ZFUtilities.h>
#import <ArtCommon/UIImageView+ZFCache.h>
#import <MediaPlayer/MediaPlayer.h>
#import <ArtCommon/ZFVolumeBrightnessView.h>
#import <ArtCommon/ZFSmallFloatControlView.h>
#import "NSBundle+ArtPodBundle.h"
#if __has_include(<ArtMediaPlayer/ZFPlayer.h>)
#import <ArtCommon/ZFPlayer.h>
#else
#import "ZFPlayer.h"
#endif
...
@interface KSPlayerControlView () <ZFSliderViewDelegate>
/// 竖屏控制层的View
@property (nonatomic, strong) KSPlayerPortraitControlView *portraitControlView;
/// 横屏控制层的View
@property (nonatomic, strong) KSPlayerLandScapeControlView *landScapeControlView;
/// 加载loading
...
@implementation KSPlayerControlView
...
- (void)layoutSubviews {
    ...
    [self handleUserControl:self.isUserControlEnable];
}
...
- (void)gestureEndedPan:(ZFPlayerGestureControl *)gestureControl panDirection:(ZFPanDirection)direction panLocation:(ZFPanLocation)location {
    if (direction == ZFPanDirectionH && self.sumTime >= 0 && self.player.totalTime > 0) {
        ...
        self.controlSeekto(self.sumTime);
        self.sumTime = 0;
    }
}
...
- (void)handleUserControl:(BOOL)isUserControlEnable {
    self.portraitControlView.isUserControlEnable = isUserControlEnable;
    self.landScapeControlView.isUserControlEnable = isUserControlEnable;
    
    self.portraitControlView.isUserControlEnable = isUserControlEnable;
    self.landScapeControlView.isUserControlEnable = isUserControlEnable;
    
    self.closeBtn.hidden = !isUserControlEnable;
    self.player.disableGestureTypes = isUserControlEnable ? ZFPlayerDisableGestureTypesNone :ZFPlayerDisableGestureTypesDoubleTap | ZFPlayerDisableGestureTypesPan | ZFPlayerDisableGestureTypesPinch;
}
...
- (void)setIsUserControlEnable:(BOOL)isUserControlEnable {
    _isUserControlEnable = isUserControlEnable;
    
    [self handleUserControl:_isUserControlEnable];
}
...
- (KSPlayerPortraitControlView *)portraitControlView {
    if (!_portraitControlView) {
        @weakify(self)
        _portraitControlView = [[KSPlayerPortraitControlView alloc] init];
        ...
        _portraitControlView.sliderValueChanged = ^(CGFloat value) {
            @strongify(self)
            [self autoFadeOutControlView];
            self.controlSeekto(self.player.totalTime * value);
        };
        _portraitControlView.controlPlayOrPause = ^(BOOL isPlaying) {
            @strongify(self);
            self.controlPlayOrPause(isPlaying);
        };
    }
    return _portraitControlView;
}

- (KSPlayerLandScapeControlView *)landScapeControlView {
    if (!_landScapeControlView) {
        @weakify(self)
        _landScapeControlView = [[KSPlayerLandScapeControlView alloc] init];
        ...
        _landScapeControlView.controlPlayOrPause = ^(BOOL isPlaying) {
            @strongify(self);
            self.controlPlayOrPause(isPlaying);
        };
    }
